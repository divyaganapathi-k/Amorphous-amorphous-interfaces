...
clearvars -except D R n God
coorleast=[];
f=find(R(:,4)==God(2,2));
coorleast(:,1:4)=R(f,1:4);
...
%         plot(mostmob1(:,1),mostmob1(:,2),'b');
        commonparticles=vertcat(commonparticles,mostmob);
    end
    l1=length(commonparticles);
    l2=length(mostmob);
    l=l1-l2;
    commonparticles=commonparticles(1:l2,:);
    H=intersect(commonparticles,mostmob,'rows');
    scatter(H(:,1),H(:,2),'m','filled');
    hold off
    h=gcf;
    saveas(h,strcat('F:\testimages-W3-interfaces\W4-test1-mostmobile\',num2str(i),'.tif'));
end