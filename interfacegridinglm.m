...
%     F=find(B>=0.9);
    F=find(B<=0.5 & B>=0.4);
%     F=find(B<=0.2 & B~=0);
    [C(:,1),C(:,2)]=ind2sub([h2,h1],F);
    C(:,1)=C(:,1);
    C(:,2)=C(:,2);
    C(:,3)=((C(:,1)-m2).^2+(C(:,2)-m1).^2).^(0.5);
    for k=1:1:length(C)
        a=atan((abs(C(k,2)-m1)/abs(C(k,1)-m2)));
        if  a>(2*pi)
            a=a-(2*pi);
        end
        if C(k,1)>=m2  && C(k,2)>=m1                                     %first quadrant
            C(k,4)=a;
        elseif C(k,1)<=m2 && C(k,2)>=m1                              % second quadrant
            C(k,4)=pi-a;
        elseif  C(k,1)<=m2 && C(k,2)<=m1                              %third quadrant
             C(k,4)=pi+a;
        elseif C(k,1)>=m2 && C(k,2)<=m1                              %fourth quadrant
             C(k,4)=(2*pi)-a;         
        end
    end
    ...
    for theta=angle:angle:2*pi
        ...
        if isempty(F1)==0
            ...
            D1=sortrows(D,1);
            if D(1,1)~=0
                mostmob(j,1)=((C(D1(1,2),2))*(boxsiz)*(sigmas))-((boxsiz/2)*(sigmas));
                mostmob(j,2)=((C(D1(1,2),1))*(boxsiz)*(sigmas))-((boxsiz/2)*(sigmas));
                mostmob(j,3)=C(D1(1,2),4);
                mostmob(j,4)=D1(1,1);
                mostmob(j,5)=i;
            else
                mostmob(j,1)=((C(D1(2,2),2))*(boxsiz)*(sigmas))-((boxsiz/2)*(sigmas));
                mostmob(j,2)=((C(D1(2,2),1))*(boxsiz)*(sigmas))-((boxsiz/2)*(sigmas));
...
%     hold off
%     h=gcf;
%     saveas(h,strcat('C:\Users\Divya\Desktop\codes\test\',num2str(i),'.tif'));
end
