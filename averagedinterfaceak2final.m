...
% (n-(2*t))
for i=1:1:max(FinalCoordinates(:,3))
...
s=((meanr-value)*(pi/180)):((meanr-value)*(pi/180)):((meanr-value)*2*pi); 
Amplitude2=zeros(181,2);
% Amplitude2(:,1)=(1/(meanr-value)):(1/(meanr-value)):(360*(1/(meanr-value)));
% Amplitude2(:,1)=(2*pi)./s;

k_max = 180/(2*pi*(meanr-value));
Amplitude2(:,1)= ((0:180)*k_max/180)';
% j=0;
fluctuations=[];
for i=1:1:(length(f))
...
%     Z=(abs(F)/length(C(:,2)));
    Z=Z.^2;
    Amplitude2(:,2)=Amplitude2(:,2)+Z(1:181,:);
%     j=j+1;
    C(:,3)=i;
