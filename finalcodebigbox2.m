...
% parfor i=1:1:n
%     F=find(R(:,3)==i);
%     N(:,:,i)=hist3(R(F,1:2),'Edges',{((addon):((boxsiz)*sigmas):x) ((0):((boxsiz)*sigmas):y)});
% end
...
m1=58;
m2=23;
x1=m1*(boxsiz)*(sigmas)-((boxsiz/2)*(sigmas))+(0);
y1=m2*(boxsiz)*(sigmas)-((boxsiz/2)*(sigmas))+(addon);
% clear AA BB CC
...
%     B=N2(:,:,dummy(i)); 
    ...
    E=[];
    E(:,1)=D(:,1)*(boxsiz)*(sigmas)-((boxsiz/2)*(sigmas))+(addon);
    E(:,2)=D(:,2)*(boxsiz)*(sigmas)-((boxsiz/2)*(sigmas))+(0);
    E(:,3)=D(:,3);
    E1=[]; E2=[];
    E1=sortrows(E,3);
    E2=unique(E1,'rows');
