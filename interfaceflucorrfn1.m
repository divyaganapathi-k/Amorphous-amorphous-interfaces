...
%     load(strcat('D:\Final Data\W3\W3final-shiftedinterface7',num2str(ii),'.mat'));
...
filename=(strcat('D:\Final Data\1ta-data analysis\W3\W3-final-ak2-0.67-0.15-box-',num2str(m1),'-',num2str(m2),'.mat'));
% filename=('D:\Final Data\W3\W3-7t-final.mat');
load(filename);
FinalCoordinates=[];
...
thetaend=min(C(:,2));
number=thetaend;
% number=floor(thetaend*(180/pi));
correlation=zeros((number+1),2);
theta2=theta1(1:number);
theta2=theta2';
correlation(2:end,1)=(meanr-value)*theta2;
for i=1:1:length(f)
    A=find(FinalCoordinates(:,3)==f(i));
    B=FinalCoordinates(A,1:2);
    B=sortrows(B,1);
    ff5=find(B(:,1)>0.1047);
    B=B(ff5,:);
    ff4=find(B(:,1)<=thetaend);
    B=B(ff4,:);
    C=autocorr(B(:,2),number);
    correlation(:,2)=correlation(:,2)+C;
end
correlation(:,2)=correlation(:,2)/length(f);
